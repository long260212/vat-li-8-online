...
import { BookOpen, Layers, Clock, Cpu, Award, User, Flame, GraduationCap, Menu, X, Sparkles, Sun, Moon, Key, ClipboardList, MessageSquare, AlertTriangle, LogOut, ShieldCheck, FlaskConical } from 'lucide-react';
...
import { hasStoredAccountForUser } from './auth';
...
export default function App() {
  const [currentUser, setCurrentUser] = useState<AppUser | null>(() => {
    try {
      ...
      const parsedUser = JSON.parse(saved) as AppUser;
      if (hasStoredAccountForUser(parsedUser)) return parsedUser;

      // Old versions allowed entering the app without an account. From this version,
      // a saved session is valid only when a real account exists in the account registry.
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/80 sticky top-0 z-40 shadow-xs transition-colors duration-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <button
              id="tab-virtual-lab"
              onClick={() => handleNavigate('virtual-lab')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                currentTab === 'virtual-lab'
                  ? 'bg-cyan-50 text-cyan-700 dark:bg-cyan-950/40 dark:text-cyan-400'
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/40'
              }`}
            >
              <FlaskConical size={14} /> Thí nghiệm
            </button>
            <button
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 shadow-lg px-4 pt-2 pb-4 space-y-1 animate-fade-in z-30 relative transition-colors duration-200">
          ...
          <button
            onClick={() => handleNavigate('virtual-lab')}
            className={`w-full text-left px-4 py-3 rounded-xl text-sm font-semibold flex items-center gap-2.5 cursor-pointer ${
              currentTab === 'virtual-lab' ? 'bg-cyan-50 text-cyan-700 dark:bg-cyan-950/40 dark:text-cyan-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/40'
            }`}
          >
            <FlaskConical size={16} /> Thí nghiệm ảo
          </button>
          <button
            onClick={() => handleNavigate('exercise-bank')}
            className={`w-full text-left px-4 py-3 rounded-xl text-sm font-semibold flex items-center gap-2.5 cursor-pointer ${
